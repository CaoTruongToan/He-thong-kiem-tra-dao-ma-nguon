...
<template>
  ...
  <DynamicScroller v-if="topComparisons.length > 0" class="scroller" :items="topComparisons" :min-item-size="48">
        ...
        <table class="inside-table">
          <tr :class="{ 'selectableEven': item.id % 2 === 0, 'selectableOdd': item.id % 2 !== 0 }">
            ...
            <td class="td6">
              ...
              " alt=">>" src="@/assets/keyboard_double_arrow_down_black_18dp.svg" @click="toggleDialog(item.id - 1)" />
            </td>
            <GDialog v-if="
              isInCluster(
                item.firstSubmissionId,
